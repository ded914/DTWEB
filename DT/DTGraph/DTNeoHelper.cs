...
ORDER BY ht.index
WITH f,s,hs,
CASE WHEN t IS NOT NULL THEN collect({nodetype: 'term', title: t.name, index: ht.index, parent: s.name}) ELSE [] END as c_terms
ORDER BY hs.index
WITH collect({title: s.name, nodetype: 'slot', index:hs.index,  nodes: c_terms, parent: f.name}) as c_slotrows
MATCH (f:Frame{name: 'Complaints' })-[ltf:LINK_TO_FRAME]->(sf:Frame)
WITH ltf, f, c_slotrows + collect({title: sf.name, nodetype: 'framelink', index: ltf.index, parent: 'frame'}) as c_allrows
WITH f, c_allrows
